...
class MultiTP extends PluginBase implements Listener {
	...
	*/
	...
	public function onEnable(){
		...
		$this->getLogger()->info("[LegoCraft] >> Load MultiTP plugin by Minifixio ...");
	...
	public function playerBlockTouch(PlayerInteractEvent $event){
		...
		if($event->getBlock()->getID() == $this->getConfig()->get("block")){
			if(count($this->positions) == 0){
				
				//If there are no positions set :
				$event->getPlayer()->sendMessage("[LegoCraft] Sorry, there is no teleport position yet set");
			}
			else{
				
				//Teleport sender to a random position
				$event->getPlayer()->teleport($this->getRandomLocation($event->getPlayer()->getLevel()->getName()));
				$event->getPlayer()->sendMessage("[LegoCraft] Let's Play!!!");
			}
		}
		else
		if($event->getBlock()->getID() == Item::SIGN_POST || $event->getBlock()->getID() == Block::SIGN_POST || $event->getBlock()->getID() == Block::WALL_SIGN){
			...
			if($sign[0]=='[KitPvP]'){
				if(empty($sign[1]) !== true){
					$worldName = $sign[1];
					$event->getPlayer()->sendMessage("[LegoCraft] Teleportation to '".$worldName."'");
					if(Server::getInstance()->loadLevel($worldName)){
						$targetPosition = $this->getRandomLocation($worldName);
						if($targetPosition == NULL){
							$event->getPlayer()->sendMessage("[LegoCraft] No random location found in '".$worldName."'. Teleporting to its spawn.");
						...
						$event->getPlayer()->sendMessage("[LegoCraft] World '".$worldName."' doesn't exist.");
					}
				}
			}
		}
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args){
		switch($command->getName()){
			...
			case "mtpset":
				...
				$sender->sendMessage("[LegoCraft] A new teleportation target was added at your position !");
				$sender->sendMessage("[LegoCraft] There are : " . $this->getNumberOfPositions() . " teleportation targets.");
				return true;
				
			//rtpreset command to reset all random spawn positions
			case "mtpreset":
				$this->resetPositions();
				$sender->sendMessage("[LegoCraft] All random spawn positions have been removed!");
				return true;
				
			//rtpinfo to have all informations about the plugin :)
			case "mtpinfo":
				$sender->sendMessage("[LegoCraft] This plugin was created by Minifixio. I've got a Youtube channel, you can subscribe :)");
				$sender->sendMessage("[LegoCraft] Commands : /mtpset ( to set a random spawn position )  and  /mtpreset ( to reset all random spawn positions.");
				$sender->sendMessage("[LegoCraft] The default teleporting block is the sponge block, but you can change this in the code of the plugin.");
				$sender->sendMessage("[LegoCraft] Perhaps there will be an update to modify the block of teleportation.");
                return true;
				$sender->sendMessage("[LegoCraft] If you have suggestions or bugs, please report this to me !");
